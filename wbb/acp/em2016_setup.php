...
if (file_exists("./lib/em2016_install.lock")) {
	die("Bitte l&ouml;schen Sie die Datei acp/lib/em2016_install.lock, um die Installation erneut ausf&uuml;hren zu k&ouml;nnen!");
}
...
print "<h2><center>EM2016-Tippspiel $version Installation</center></h2>\n<br />\n";

if (!$step) {

	informationPage('<b>Herzlich Willkommen bei der Einrichtung des EM2016-Tippspiels f&uuml;r das WBB 2.3.x</b><br /><br />
  <table>
  <tr>
  ...
  <tr>
   <td>Sprachdatei "/acp/em2016_de.lng"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../acp/em2016_de.lng")) ? ('red') : ('lime')) . '">' . ((is_file("../acp/em2016_de.lng")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
  <tr>
   <td>Variablendatei "/acp/em2016.wbb"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../acp/em2016.wbb")) ? ('red') : ('lime')) . '">' . ((is_file("../acp/em2016.wbb")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
  <tr>
   <td>SQL-Datei "/acp/em2016.sql"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../acp/em2016.sql")) ? ('red') : ('lime')) . '">' . ((is_file("../acp/em2016.sql")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
  <tr>
   <td>Haupt-Datei "/em2016.php"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../em2016.php")) ? ('red') : ('lime')) . '">' . ((is_file("../em2016.php")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
  <tr>
   <td>ACP-Datei "/acp/em2016_admin.php"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../acp/em2016_admin.php")) ? ('red') : ('lime')) . '">' . ((is_file("../acp/em2016_admin.php")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
  <tr>
   <td>ACP-Template "/acp/templates/em2016_info.htm"</td>
   <td>Ja</td>
   <td><span style="color: ' . ((!is_file("../acp/templates/em2016_info.htm")) ? ('red') : ('lime')) . '">' . ((is_file("../acp/templates/em2016_info.htm")) ? ('Ja') : ('Nein')) . '</span></td>
  </tr>
</table>
 <p><i>Sollten eine oder mehrere Voraussetzungen nicht erf&uuml;llt sein, kann ein einwandfreier Betrieb des EM2016-Tippspiels nicht gew&auml;hrleistet werden.</i></p>
 <br />
 ');
	...
	$filefehlt = 0;
	$filefehlta = '';
	if (!is_file("./em2016_de.lng")) {
		$filefehlt = 1;
		$filefehlta .= "<li>Sprachdatei \"/acp/em2016_de.lng\" nicht gefunden</li>";}
	if (!is_file("./em2016.wbb")) {
		$filefehlt = 2;
		$filefehlta .= "<li>Rechtedatei \"/acp/em2016.wbb\" nicht gefunden</li>";}
	if (!is_file("./em2016.sql")) {
		$filefehlt = 3;
		$filefehlta .= "<li>SQL-Datei \"/acp/em2016.sql\" nicht gefunden</li>";}
	if (!is_file("../em2016.php")) {
		$filefehlt = 4;
		$filefehlta .= "<li>Haupt-Datei \"/em2016.php\" nicht gefunden</li>";}
	if (!is_file("./em2016_admin.php")) {
		$filefehlt = 5;
		$filefehlta .= "<li>ACP-Datei \"acp/em2016_admin.php\" nicht gefunden</li>";}
	if (!is_file("./templates/em2016_info.htm")) {
		$filefehlt = 6;
		$filefehlta .= "<li>ACP-Template \"/acp/templates/em2016_info.htm\" nicht gefunden</li>";}
	...
	if ($filefehlt > 0) {
		...
		print $filefehlta . "<br />\n";
		print "Bitte laden Sie erst alle Dateien des EM2016-Tippspiels $version in die entsprechenden Verzeichnisse \n";
...
// # L&ouml;schen der alten Eintr&auml;ge
	...
	informationPage('<b>Die ACP-Templates des EM2016-Tippspiels werden nun vorbereitet.</b>');
...
if ($step == "langask") {
	...
	informationPage('<b>Das Sprachpaket des EM2016-Tippspiels installieren.</b>');
	...
	print "Das EM2016-Tippspiel $version kann auf deutsch dargestellt werden.<p />";
	print "Wenn Sie in Ihrem Forum mehrere Sprachpakete installiert haben, k&ouml;nnen Sie ";
	print "hier bestimmen, in welchen Sprachpaketen das EM2016-Tippspiel $version dargestellt werden soll.<p />";

	print "Normalerweise erkennt das Programm das selbst und Sie brauchen hier nichts zu &auml;ndern.<p />";

	print "In welchen Ihrer Sprachpakete soll das EM2016-Tippspiel auf  <b>deutsch</b> erscheinen?<br />";
...
if ($step == "langinst") {
	...
	if (empty($languagepackids_de)) {
		...
		print "Ohne Sprachvariablen kann das EM2016-Tippspiel nicht benutzt werden. Sie k&ouml;nnen die<br />\n";
		print "Sprachvariablen des EM2016-Tippspiels entweder sp&auml;ter von Hand im ACP importieren, oder<br />\n";
...
if ($step == "groups") {
	...
	$variableimport = new variableimport("./em2016.wbb");
...
if ($step == "database") {
	...
	// FERTIG !!!
	// ####################################################################
	$fp = @fopen("./lib/em2016_install.lock", "w+");
	fclose($fp);
	...
	print "Sie k&ouml;nnen das EM2016-Tippspiel $version jetzt benutzen.<p>";
	...
	print "<pre>em2016_setup.php, em2016_de.lng, em2016.wbb, em2016.sql</pre>";
